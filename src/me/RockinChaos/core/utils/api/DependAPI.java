...
public class DependAPI {
    ...
    */
	public void sendUtilityDepends() {
		...
		if (Bukkit.getServer().getPluginManager().isPluginEnabled("Vault") && !this.getVault().vaultEnabled()) {
			ServerUtils.logDebug("{VaultAPI} An error has occured while setting up enabling Vault support, no economy plugin detected."); 
		}
	}
    ...
    */
    public static DependAPI getDepends() { 
        if (depends == null) { depends = new DependAPI(); }
        return depends; 
    }
}