...
public class Page {
    ...
    * @param slot - The slot that relates to the button that was clicked for the page.
    */
	public void handleTyping(final PrepareAnvilEvent event, int slot) {
		Button button = this.buttons.get(slot);
		button.onTyping(event);
	}
   ...
   /**
    * Executes the typing event method for the specified slots button.
    * 
    * @param slot - The slot of the clicked button.
    * @return The typing event was successfully passed through to the button.
    */
	public boolean typingEvent(final int slot) {
		if (slot <= this.buttons.size()) {
			return this.buttons.get(slot).typingEvent();
		}
		return false;
	}
}