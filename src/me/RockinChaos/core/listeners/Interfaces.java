...
public class Interfaces implements Listener {
   ...
   /**
    * Handles the typing action for the virtualInventory.
    * 
    * @param event - PrepareAnvilEvent
    */
	//@EventHandler(priority = EventPriority.LOWEST, ignoreCancelled = false)
	@SuppressWarnings("unused")
	private void onType(final PrepareAnvilEvent event) {
		InventoryHolder holder = event.getInventory().getHolder();
		if (holder instanceof Interface && event.getInventory().getType().equals(InventoryType.ANVIL)) {
			//ServerUtils.logDev("Anvil Typing Ready");
			((Interface) holder).onTyping(event);
    		//this.expiredInventory = ((Interface) holder);
		}
	}
